...
class Dignity:
    def __init__(self, memory_instance, discord_client):
        ...
        self.agents = {
            ...
            "generate": ChatAgent(agent_name='Reflection'),
            "reflect": ChatAgent(agent_name='GenerateResponse'),
        }
    ...
    def run_agent(self, agent_name):
        max_reruns = 3
        self.logger.log(f"Running {agent_name.capitalize()} Agent... Message:{self.message['message']}", 'info', 'Dignity')
        ...
        while 'error' in result:
            ...
            self.logger.log(result_message, 'error', 'Dignity')
    ...
    def run_cognition_process(self):
        ...
        while True:
            ...
            if iteration_count > max_iterations:
                self.logger.log("Maximum iteration count reached, forcing response", 'warning', 'Dignity')
                break

            reflection = self.cognition['reflect']
            self.logger.log(f"Handle Reflection: {reflection}", 'debug', 'Dignity')
            ...
            if "Choice" in reflection:
                ...
                self.logger.log("No 'Choice' found in reflection. Handling as parsing error.", 'warning', 'Dignity')
                self._handle_parsing_error(reflection)
                break  # Exit loop after handling parsing error
    ...
    def _determine_action(self, reflection):
        ...
        for key, value in actions.items():
            if key in choice:
                self.logger.log(value['log'], 'info', 'Dignity')
                return value['action']

        self.logger.log(f"Unknown choice in reflection: '{choice}'", 'warning', 'Dignity')
        return 'unknown'

    def _handle_parsing_error(self, reflection):
        self.logger.log(f"Parsing Error in Reflection: {reflection}\nRerunning reflection...", 'error', 'Dignity')
        self.run_agent('reflect')
