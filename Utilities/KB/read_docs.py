...
try:
    all_entries = storage.query_memory(collection_name='docs', query={})
    if all_entries is None:
        print("Warning: query_memory returned None. The database might be empty or there might be an issue with the Chroma backend.")
        all_sources = set()
    else:
        all_sources = set(metadata['Source'] for metadata in all_entries.get('metadatas', []) if metadata and 'Source' in metadata)
