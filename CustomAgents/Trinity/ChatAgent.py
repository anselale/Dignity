...
class ChatAgent(Agent):
    ...
    def process_data(self):
        ...
        self.template_data['self_scratchpad'] = self.template_data['cognition']['self_scratchpad']
        ...
        while generation_attempts < max_generation_attempts:
            ...
            try:
                ...
                if expected_format:
                    while parsing_attempts < max_attempts:
                        ...
                        if parsed_result:
                            ...
                            self.parsed_result['result'] = result_str
                            return
                        else:
                            parsing_attempts += 1
                ...
                if generation_attempts >= max_generation_attempts:
                    ...
                    self.parsed_result = {'error': error_message}
                    return
                else:
                    self.logger.log("Re-running LLM to generate a new response.", 'info')
                    # Rerun the LLM to get a new response
                    self.run_model()
                    # The result of LLM is stored in self.parsed_result, so we can continue
    ...
    def get_response_format_markdown(self):
        ...
        """
        response_format = self.prompt_template['system'].get("Response Format")
        if not response_format:
            response_format = self.prompt_template['user'].get("Response Format")
    ...
    def handle_parsing_error(self, result_str, exception):
        ...
        self.parsed_result = {'error': f'Exception during parsing: {str(exception)}'}
